...
#Classe que monta a arvore recebendo as arestas computadas por prim

class Tree:
    def montar_prim(self,all_points,link,custo):
        #Essa classe tá recebendo dois arrays que indicam os links entre os vertices e os pesos
        self.arestas = []
        #aqui eu vejo o array de link e o de custo e monto as arestas resultantes
        for i in range(0,len(link)):
            if(link[i] != -1):#Se existe conexão
                aresta = (all_points[i],all_points[link[i]],custo[i]) #Crio uma aresta na forma (Ponto a,ponto b,peso)
                self.arestas.append(aresta)
        
        #aqui eu monto as adjancencias para cada vertice
        #Na forma (ponto,custo pra ele)
        self.lista_adjacencias = []
        #Cada ponto pode ter mais de uma conexão aí eu exploro todas as arestas da árvore e coloco aqui
        for i in range(0,len(all_points)):
            adjacencia = []
...
def mstPrim(G,all_points,start):
    ...
    while(0 in fila):
        atual = extrair_minimo(fila,pesos) #procuro o vertice não visitado com menor valor (atual = index de algum vertice)
...
def Main():
    ...
    V = lerVertices(file,n_obstacles) #Vertices agrupados com os obstaculos respectivos
        #V é uma lista formada por 3 listas (Os obstaculos) com os seus respectivos pontos
    ...
    T_prim = mstPrim(G,all_points,start) #Essa função vai gerar as arestas da MST e retornar um objeto da classe Tree
